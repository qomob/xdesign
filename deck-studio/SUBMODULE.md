...
> **Note for XDesign agents**: This file is preserved as the upstream `html-ppt-skill` entry point. Within the XDesign skill it lives at `deck-studio/SUBMODULE.md` (renamed from `SKILL.md` to keep a single skill entry point). The authoritative router is the parent skill's [`../SKILL.md`](../SKILL.md); read it first to decide between Mode 1 (this submodule), Mode 2 (visual design), or Mode 3 (animation). The body of this file is unchanged from the upstream version.
