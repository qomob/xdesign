...
deck-studio 的 18 个主题在 `deck-studio/assets/themes/`。`SKILL.md Mode 1` 章节里列的主题名（如 `tokyo-night`, `vaporwave`, `xiaohongshu-white`）部分还未物理下载。**Agent 在推荐主题时，必须先检查 `ls deck-studio/assets/themes/`**，找不到的主题不能虚构。

目前**已下载的主题**（18 个）：
- academic-paper, arctic-cool, aurora, bauhaus, blueprint
- catppuccin-latte, catppuccin-mocha, corporate-clean, cyberpunk-neon
- dracula, editorial-serif, engineering-whiteprint, glassmorphism
- gruvbox-dark, japanese-minimal, magazine-bold, memphis-pop
- midcentury
...
- [ ] `ls deck-studio/assets/themes/` — 18 个 CSS 都在
- [ ] `ls deck-studio/templates/full-decks/` — 5 个 full-deck
